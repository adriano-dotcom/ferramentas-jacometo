...
export default function Page() {
  ...
  useJobPolling(
    ...
    setJob,
    '/mitsui-inadimplentes'
  )

  async function executar() {
    setExecutando(true); setErroEnvio('')
    try {
      const res  = await fetch('/api/rpa/mitsui-inadimplentes/executar', { method: 'POST' })
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '0.5px solid var(--border)', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 500 }}>Mitsui — Parcelas Pendentes</span>
        {emProcesso && (
      ...
      <div style={{ maxWidth: 540, margin: '0 auto', padding: '2rem 1rem' }}>
        <h1 style={{ fontSize: 20, fontWeight: 500, marginBottom: 4 }}>Parcelas Pendentes — Mitsui (MSIG)</h1>
        <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: '1.5rem', lineHeight: 1.5 }}>Acessa o Kit Online Mitsui, consulta por período de vencimento e extrai a aba Pendentes.</p>
        {!job && (
          <>
            <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 12, overflow: 'hidden', marginBottom: 14 }}>
              <div style={{ padding: '12px 14px', borderBottom: '0.5px solid var(--border)' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 500 }}>www4.msig.com.br/kitonline</div>
                <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 2 }}>FINANCEIRO → Parcelas e 2ª Via de Boleto → aba Pendentes</div>
              </div>
              <div style={{ padding: '12px 14px', borderBottom: '0.5px solid var(--border)' }}>
                <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 6 }}>Etapas</div>
                {<div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 12, color: 'var(--text-2)' }}><span style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--bg)', border: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--text-3)', flexShrink: 0 }}>1</span>Login Mitsui Kit Online</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 12, color: 'var(--text-2)' }}><span style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--bg)', border: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--text-3)', flexShrink: 0 }}>2</span>FINANCEIRO → Parcelas e 2ª Via</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 12, color: 'var(--text-2)' }}><span style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--bg)', border: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--text-3)', flexShrink: 0 }}>3</span>Pesquisa por vencimento</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 12, color: 'var(--text-2)' }}><span style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--bg)', border: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--text-3)', flexShrink: 0 }}>4</span>Aba Pendentes + extração</div>
            ...
            <div style={{ padding: '9px 12px', marginBottom: 14, background: '#E6F1FB', border: '0.5px solid #185FA5', borderRadius: 8, fontSize: 12, color: '#0C447C' }}>ℹ️ Portal antigo (estilo clássico). Nomes de segurados podem aparecer truncados — o sistema captura o texto completo.</div>
            <div style={{ padding: '8px 12px', marginBottom: 14, background: '#E6F1FB', border: '0.5px solid #185FA5', borderRadius: 8, fontSize: 12, color: '#0C447C' }}>
              ℹ️ Corretor: 0104339 · Produto: Todos · Período: Vencimento de 01/01 até hoje.
            </div>

...
export default function GiacometYelumPage() {
  ...
  useJobPolling(
    ...
    setJob,
    '/giacomet-yelum-inadimplentes'
  )

  async function executar() {
    setExecutando(true); setErroEnvio('')
    try {
      const res = await fetch('/api/rpa/giacomet-yelum-inadimplentes/executar', { method: 'POST' })
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '0.5px solid var(--border)', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 500 }}>Giacomet — Yelum</span>
        {emProcesso && (
      ...
      <div style={{ maxWidth: 540, margin: '0 auto', padding: '2rem 1rem' }}>
        <h1 style={{ fontSize: 20, fontWeight: 500, marginBottom: 4 }}>Parcelas em Atraso — Giacomet Yelum</h1>

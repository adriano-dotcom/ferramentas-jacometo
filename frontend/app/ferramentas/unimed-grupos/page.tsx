...
export default function UnimedGruposPage() {
  ...
  async function processar() {
    ...
    try {
      ...
      const houveMudanca = data.adicionados > 0 || data.removidos > 0
      const resultados: any[] = []

      resultados.push({
        nome: `${data.total} grupo(s) na lista`,
        sub: data.semDia > 0 ? `${data.semDia} sem dia de fatura definido` : null,
        status: 'OK',
      ...
      if (houveMudanca) {
        if (data.adicionados > 0) {
          resultados.push({
            nome: `${data.adicionados} grupo(s) adicionado(s)`,
            status: 'OK', label: null, orientacao: null, erro: null, tipo: null,
          })
        }
        if (data.removidos > 0) {
          resultados.push({
            nome: `${data.removidos} grupo(s) removido(s)`,
            status: 'AVISO',
            label: 'Grupos removidos da lista',
            orientacao: 'Verifique se a remoção foi intencional.',
            erro: null, tipo: null,
          })
        }
        resultados.push({
          nome: data.emailEnviado ? 'Notificação enviada por email' : 'Email não enviado',
          status: data.emailEnviado ? 'OK' : 'AVISO',
          label: data.emailEnviado ? null : 'Falha no envio',
          orientacao: data.emailEnviado ? null : 'Verifique configurações de SMTP.',
          erro: null, tipo: null,
        })
      } else {
        resultados.push({
          nome: 'Lista sem alterações',
          sub: 'Mesmos grupos da importação anterior',
          status: 'OK', label: null, orientacao: null, erro: null, tipo: null,
        })
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: '1.5rem' }}>Envie a planilha com os números de grupo. O sistema salva a lista e notifica se houver mudanças.</p>
        {!job && (
          <>
            ...
            <button onClick={processar} disabled={!arquivo || enviando} style={{ width: '100%', padding: '11px', background: !arquivo || enviando ? 'var(--border)' : '#1D9E75', color: !arquivo || enviando ? 'var(--text-3)' : '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: !arquivo || enviando ? 'default' : 'pointer' }}>
